...
class ObservableRequestHandlerTracerImpl {
   ...
   */
    ...
    // Idempotent: Cbjs threads ops through a single try/catch (rather than the
    // upstream wrapAsync isolation), so end()/endWithError() may both be reached
    // on the same handler. Guard to ensure the span is ended exactly once.
...
class ObservableRequestHandlerNoOpMeterImpl {
  private _opType: OpType;
  constructor(opType: OpType, _observabilityInstruments: ObservabilityInstruments) {
    this._opType = opType;
  }
...
class ObservableRequestHandlerMeterImpl {
  ...
  constructor(opType: OpType, observabilityInstruments: ObservabilityInstruments) {
    this._opType = opType;
    this._serviceName = serviceNameFromOpType(opType);
  ...
  setRequestKeyValueAttributes(cppDocId: CppDocumentId): void {
    this._attrs = getAttributesForKeyValueOpType(this._opType as KeyValueOp, cppDocId);
  }
  ...
  processEnd(clusterName?: string, clusterUUID?: string, error?: any): void {
    ...
    const duration = hiResTimeToMicros(getHiResTimeDelta(this._startTime, endTime));
   ...
   * @internal
   */
  reset(opType: OpType, clusterName?: string, clusterUUID?: string, error?: any): void {
...
export class WrappedSpan implements RequestSpan {
  ...
  maybeCreateEncodingSpan(encodeFn: () => [Buffer, number]): [Buffer, number] {
    ...
    try {
      ...
      // we wait to set the end time until we process the underylying
      // core span so that we can add the cluster_[name|uuid] attributes
      this._encodingSpan = new WrappedEncodingSpan(encodingSpan, timeInputToHiResTime());
    }
  }
  ...
  _buildDispatchCoreSpan(
    ...
    for (const [attrName, attrVal] of Object.entries(coreSpan.attributes ?? {})) {
      newSpan.setAttribute(attrName, attrVal);
    }
    const coreSpanEndTime = getCoreSpanEndTime(coreSpan.end);
    this._endTimeWatermark = getLatestTime(this._endTimeWatermark, coreSpanEndTime);
   ...
   */
  processCoreSpan(coreSpan: CppWrapperSdkSpan): void {
    this._maybeSetAttributeFromCoreSpan(coreSpan, CppOpAttributeName.ClusterName);
    this._maybeSetAttributeFromCoreSpan(coreSpan, CppOpAttributeName.ClusterUUID);
    this._maybeSetAttributeFromCoreSpan(coreSpan, CppOpAttributeName.RetryCount, true);
