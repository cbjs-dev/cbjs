...
import type { MutateInArrayAddUniquePath } from './mutationOperations.types.js';

describe('mutateIn arrayAddUnique', async () => {
  ...
  type TestPaths<Doc extends object, T extends Record<MakeTestPaths<Doc>, boolean>> = {
    [Path in keyof T]: [
      T[Path],
      Path extends MutateInArrayAddUniquePath<Doc> ? true : false,
    ];
  };
