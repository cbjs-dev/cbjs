...
import type { If, IsExactly, IsNever, Join, Primitive, TrySafe, UnionToTuple } from '@cbjs/shared';
...
export type TypeOf<T> =
  T extends unknown ?
    T extends string ? If<IsExactly<T, string>, 'string', `string::${T}`> :
    T extends boolean ? T extends true ? 'boolean::true' : 'boolean::false' :
    T extends null ? 'null' :
    T extends undefined ? 'undefined' :
    T extends ReadonlyArray<unknown> ? `array` :
    T extends { [K: keyof any]: unknown } ? `object(${Join<TupleFilter<UnionToTuple<keyof T>, string, true>, ', '>})` :
    'unknownType' :
  never
;
 ...
 */
export type WrapEach<Tuple extends ReadonlyArray<string>, Left extends string = '', Right extends string = ''> =
...
export type WritableKeys<T extends object> = {
  [K in keyof T]: IsExactly<Pick<T, K>, {readonly [RO in K]: T[K]}> extends false ? K : never
}[keyof T];
 ...
 */
export type CircularReferences<T, Parents extends ReadonlyArray<unknown> = []> =
 ...
 */
export type ReferencesItself<T> =
  TrySafe<(
