 ...
 */
type ObjectAccessor<K extends AccessibleKey, IsRoot extends boolean> = IsRoot extends true ? K : `.${K & string}`;
 ...
 */
export type IsArrayIndexAccessor<T> = [ExtractArrayIndexAccessor<T>] extends [never] ? false: true
 ...
 */
type PathAhead<T extends object, Key extends AccessibleKey & keyof T, PropBags extends CircularPropBag> =
  T[ResolveNegativeIndex<T, Key>] extends infer Doc ?
    Doc extends unknown ?
      Doc extends readonly unknown[] ? ArrayPath<Doc, PropBags> :
      Doc extends object ? ObjectPath<Doc, PropBags> :
      never:
    never :
  never
;
 ...
 */
type ObjectPath<Doc extends object, PropBags extends CircularPropBag> =
  BuildPath<Doc, keyof Doc & AccessibleKey, false, PropBags>
;
 ...
 */
export type SplitIntoSegments<T extends string> =
  T extends string ?
 ...
 */
type SplitSegmentIntoAccessors<T> =
  T extends `${infer Prop}[${infer Accessors}]` ?
    Prop extends '' ?
 ...
 */
export type JoinSegments<T extends readonly string[], InitialString extends string = ''> =
...
type SplitSegmentsAccessors<Segments extends readonly string[]> =
  Segments extends [infer Segment, ...(infer Rest extends readonly string[])] ?
 ...
 */
export type PopUntilProperty<Tuple extends readonly string[]> =
  Tuple extends [...(infer Rest extends readonly string[]), infer Tail] ?
...
export type MaybeMissing<T, K = never> =
  If<
    IsNever<
      | undefined extends T ? true : never
      | T extends ReadonlyArray<unknown> ?
          K extends GuaranteedIndexes<T> ? never : true :
        never
      | K extends OptionalKeys<T> ? true : never
    >,
    false,
    true
  >
 ...
 */
type ObjectSubDocument<T, Segments extends ReadonlyArray<string>, OrUndefined extends boolean> =
 ...
 */
type ArraySubDocument<T, Segments extends ReadonlyArray<string>, OrUndefined extends boolean> =
  Segments extends [infer Segment extends string, ...infer RestSegments extends string[]] ?
    Segment extends `[${infer Index}]` ? // Array access
      Extract<T, ReadonlyArray<unknown>> extends infer Arr extends ReadonlyArray<unknown> ?
        ResolveNegativeIndex<Arr, ToNumber<Index>> extends infer ResolvedIndex extends keyof Arr ?
          1 extends Partial<RestSegments>['length'] ?
            SubDocumentFromPathSegments<Arr[ResolvedIndex], RestSegments, MaybeMissing<Arr, ResolvedIndex>> :
          Arr[ResolvedIndex] | If<Or<[OrUndefined, MaybeMissing<Arr, ResolvedIndex>]>, undefined> :
        never :
      never :
    never :
  never
 ...
 */
type RootSubDocument<T, Segments extends ReadonlyArray<string>> =
  Segments extends [infer Segment extends string, ...string[]] ?
    Segment extends '' ?
      T :
    never :
  never
 ...
 */
type SubDocumentFromPathSegments<T, Segments extends ReadonlyArray<string>, OrUndefined extends boolean> =
  T extends unknown ?
...
export type ParentDocument<D, P extends string> =
  PathToParentAccessor<P> extends infer ParentAccessor extends string ?
    [ParentAccessor] extends [never] ? D : SubDocument<D, ParentAccessor> :
  never
;
 ...
 */
export type PathToClosestProperty<T extends string> =
  T extends unknown ?
...
export type PathToParentPropertyOrSelf<T extends string> =
  PathToParentProperty<T> extends infer ParentProp ?
    [ParentProp] extends [never] ? T : ParentProp :
  never
;
