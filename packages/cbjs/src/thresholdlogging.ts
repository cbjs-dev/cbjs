...
const IGNORED_PARENT_SPAN_VALUES = new Set<string>(Object.values(IgnoredParentSpan));
...
class ThresholdLoggingReporter {
  ...
  private readonly _thresholdQueues: Map<ServiceType, PriorityQueue<ThresholdLogRecord>>;
  ...
  constructor(logger: CouchbaseLogger, interval: number, capacity?: number) {
    ...
    this._thresholdQueues = new Map<ServiceType, PriorityQueue<ThresholdLogRecord>>();
...
export class ThresholdLoggingSpan implements RequestSpan {
   ...
   */
    ...
    } else if (!this._parentSpan && !IGNORED_PARENT_SPAN_VALUES.has(this._name)) {
      this._tracer.checkThreshold(this._snapshot);
    }
  }
...
export class ThresholdLoggingTracer implements RequestTracer {
  ...
  checkThreshold(spanSnapshot: ThresholdLoggingSpanSnapshot): void {
    ...
    // A threshold of 0 means "trace every operation"; only an unconfigured
    // service (undefined) disables threshold logging. Do NOT treat 0 as disabled.
    if (serviceThreshold === undefined || spanTotalDuration <= serviceThreshold) {
      return;
    }
   ...
   * @internal
   */
  _getServiceTypeThreshold(serviceType: ServiceType): number | undefined {
    const baseThreshold = this._serviceThresholds.get(serviceType);
    // An unconfigured service has no threshold (disabled). A configured value of
    // 0 is preserved as 0 ("trace everything") rather than collapsed to disabled.
