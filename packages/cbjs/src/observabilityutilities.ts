...
import binding, { CppDocumentId, CppDurabilityLevel, HiResTime } from './binding.js';
import {
  AttributeValue,
  HttpOpType,
  KeyValueOp,
...
import { QueryOptions } from './querytypes.js';
...
export function getAttributesForKeyValueOpType(
  ...
  if (durability && durability != binding.durability_level.none) {
    ...
    } else if (durability === binding.durability_level.majority_and_persist_to_active) {
      attributes[OpAttributeName.DurabilityLevel] = 'majority_and_persist_to_active';
