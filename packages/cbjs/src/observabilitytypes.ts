...
// The public observability value/types now live in @cbjsdev/shared; re-export
// them so existing `./observabilitytypes.js` importers keep working unchanged.
export {
  type AttributeValue,
  type SpanStatus,
  SpanStatusCode,
...
export function serviceNameFromOpType(opType: OpType): ServiceName {
  ...
  } else if (Object.values(DatastructureOp).includes(opType as DatastructureOp)) {
    return ServiceName.KeyValue;
  ...
  } else if (Object.values(AnalyticsMgmtOp).includes(opType as AnalyticsMgmtOp)) {
    return ServiceName.Analytics;
  } else if (Object.values(BucketMgmtOp).includes(opType as BucketMgmtOp)) {
    return ServiceName.Management;
  } else if (Object.values(CollectionMgmtOp).includes(opType as CollectionMgmtOp)) {
    return ServiceName.Management;
  } else if (
    Object.values(EventingFunctionMgmtOp).includes(opType as EventingFunctionMgmtOp)
  ) {
    return ServiceName.Eventing;
  } else if (Object.values(QueryIndexMgmtOp).includes(opType as QueryIndexMgmtOp)) {
    return ServiceName.Query;
  } else if (Object.values(SearchIndexMgmtOp).includes(opType as SearchIndexMgmtOp)) {
    return ServiceName.Search;
  } else if (Object.values(UserMgmtOp).includes(opType as UserMgmtOp)) {
    return ServiceName.Management;
  } else if (Object.values(ViewIndexMgmtOp).includes(opType as ViewIndexMgmtOp)) {
    return ServiceName.Views;
  }
