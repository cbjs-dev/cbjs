...
/**
 * Internal Cbjs debug logger (pino). Distinct from the public {@link Logger}
 * interface below, which is the pluggable SDK logger surfaced to users.
 *
 * @internal
 */
 ...
 * @internal
 */
export function setCbjsLogger(userLogger: PinoLogger) {
  logger = userLogger;
}
...
export function parseLogLevel(level: LogLevel | string): LogLevel | undefined {
  if (typeof level === 'number') {
    return level >= LogLevel.TRACE && level <= LogLevel.ERROR ? level : undefined;
  }
