...
import { If, IsExactly, IsNever, Or } from '../utils';
 ...
 */
export type IsArrayLengthFixed<T extends ReadonlyArray<unknown>> =
...
export type GuaranteedIndexes<T extends ReadonlyArray<unknown>> =
  GetArrayInfo<T> extends infer Info extends ArrayInfoShape ?
    If<Or<[Info['IsHeadStatic'], Info['IsTailStatic']]>, TupleIndexes<Info['StaticSlice']>> :
  never
;
 ...
 */
export type GetArrayInfo<T extends ReadonlyArray<unknown>, End extends 'head' | 'tail' = never, Slice extends ReadonlyArray<unknown> = []> =
  IsArrayLengthKnown<T> extends true ?
    {
      IsFullyStatic: true;
      IsHeadStatic: true;
...
export type ResolveNegativeIndex<T, K> =
  T extends ReadonlyArray<unknown> ?
    K extends -1 ? ArrayLastIndex<T> : K :
  K
;
 ...
 */
export type IsIndexRemovalStrictlyForbidden<T extends ReadonlyArray<unknown>, K extends number> =
 ...
 */
export type ExtractPrependableArray<T extends ReadonlyArray<unknown>> =
  T extends unknown ?
    [ArrayPrependElement<T>] extends [never] ? never : T
  : never
;
 ...
 */
export type ExtractAppendableArray<T extends ReadonlyArray<unknown>> =
  T extends unknown ?
    [ArrayAppendElement<T>] extends [never] ? never : T
  : never
;

export type DropTupleHead<T extends ReadonlyArray<unknown>> =
  T extends [unknown, ...infer Rest] ?
    Rest :
  []
;

export type DropTupleTail<T extends ReadonlyArray<unknown>> =
  T extends [...infer Rest, unknown] ?
    Rest :
  []
;

export type TupleTail<T extends ReadonlyArray<unknown>> =
  T extends [...ReadonlyArray<unknown>, infer Tail] ?
    Tail :
  never
;