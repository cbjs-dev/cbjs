...
// TODO set value to "never" when the CC excludes the path
//  and return "never" (or a nice error message) when the op is "remove" and the CC excludes the path

type TD = { title?: string; arr: [number, ...string[]] };
type CC = DocumentCodeCompletion<'remove', TD>;

declare function remove<Path extends OpCodeCompletionPath<'remove', TD>>(
  path: Path
): Path;
declare function removeValidate<Path>(path: IsLegalPath<'remove', CC, Path>): Path;

// remove('title');
// removeValidate('arr[1]');
...
type S1 = Split<`arr[1]`, '.', '`'>;
type S2 = Split<`arr[${number}]`, '.', '`'>;
type TS = S1 extends S2 ? true : false;

type T1 = ExtractMatchingCC<
  'get',
  [`tags[${number}]`, string] | ['tags[1]', string],
  'tags[1]'
>;
...
type TTD = Record<string, { title: string }>;
type TTT = TTD[string];
